...
pair<string_view, uint32_t> stripAsciiWhitespaceWithCount(string_view str) {
    ...
    return {str.substr(start), prefixWhitespaceCount};
}
...
void SigsRewriter::checkForUnusedComments(core::MutableContext ctx) {
    if (!methodSignatures.empty()) {
        for (const auto &[lineNumber, comments] : methodSignatures) {
            for (const auto &sig : comments.signatures) {
                if (auto e = ctx.beginError(sig.typeLoc, core::errors::Rewriter::RBSUnsupported)) {
...
void SigsRewriter::extractComments(std::string_view sourceCode) {
...
unique_ptr<parser::Node> SigsRewriter::run(unique_ptr<parser::Node> node) {
    extractComments(ctx.file.data(ctx).source());
    auto body = rewriteBody(move(node));
    checkForUnusedComments(ctx);
