    ...
    if (auto it = commentsByNode.find(node.get()); it != commentsByNode.end()) {
        for (const auto &commentNode : it->second) {
            if (absl::StartsWith(commentNode.string, "#:")) {
                ...
                // If we already consumed this comment, skip it
                if (hasConsumedComment(commentNode.loc)) {
                    continue;
                }

                // Consume the comment so it won't be picked up by subsequent calls
...
unique_ptr<parser::Node> AssertionsRewriter::maybeInsertCast(unique_ptr<parser::Node> node,
                                                             unique_ptr<parser::Node> nodeWithComments = nullptr) {
    if (node == nullptr) {
        return node;
    }
...
unique_ptr<parser::Node> AssertionsRewriter::rewriteNode(unique_ptr<parser::Node> node) {
    ...
    typecase(
        ...
        [&](parser::Send *send) {
            ...
            if (isVisibilitySend(send) || isAttrAccessorSend(send)) {
                result = move(node);
                return;
            }
