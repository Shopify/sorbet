...
pm_node_t *handleAnnotations(core::MutableContext ctx, const pm_node_t *node, pm_node_t *sigBuilder,
    ...
    for (auto &annotation : annotations) {
        if (annotation.string == "final") {
            ...
            sigBuilder = PMK::Send0(annotation.typeLoc, sigBuilder, core::Names::abstract().show(ctx.state).c_str());
            ensureAbstractMethodRaises(ctx, node, prismParser);
        } else if (annotation.string == "overridable") {
            sigBuilder = PMK::Send0(annotation.typeLoc, sigBuilder, core::Names::overridable().show(ctx.state).c_str());
        } else if (annotation.string == "override") {
            sigBuilder = PMK::Send0(annotation.typeLoc, sigBuilder, core::Names::override_().show(ctx.state).c_str());
        } else if (annotation.string == "override(allow_incompatible: true)") {
            // Create hash: {allow_incompatible: true}
            auto key = PMK::Symbol(annotation.typeLoc, core::Names::allowIncompatible().show(ctx.state).c_str());
            auto value = PMK::True(annotation.typeLoc);
            auto pair = PMK::AssocNode(annotation.typeLoc, key, value);

            vector<pm_node_t *> pairs;
            pairs.push_back(pair);
            auto hash = PMK::KeywordHash(annotation.typeLoc, pairs);

            vector<pm_node_t *> args;
            args.push_back(hash);

            sigBuilder =
                PMK::Send(annotation.typeLoc, sigBuilder, core::Names::override_().show(ctx.state).c_str(), args);
        }
    }
    return sigBuilder;
}
...
pm_node_t *MethodTypeToParserNodePrism::methodSignature(const pm_node_t *methodDef, const rbs_method_type_t *methodType,
    ...
    sigReceiver = handleAnnotations(ctx, methodDef, sigReceiver, annotations, prismParser);
    ...
    *block = (pm_block_node_t){.base = PMK::initializeBaseNode(PM_BLOCK_NODE),
    ...
    if (final != annotations.end()) {
        pm_node_t *finalSymbol = PMK::Symbol(final->typeLoc, core::Names::final_().show(ctx.state).c_str());
        if (finalSymbol) {
