...
vector<pm_node_t *> extractHelpers(core::MutableContext ctx, const vector<Comment> &annotations) {
    ...
    for (auto &annotation : annotations) {
        ...
        if (annotation.string == "abstract") {
            pm_node_t *self = PMK::Self(annotation.typeLoc);
            helperNode = PMK::Send0(annotation.typeLoc, self, "abstract!");
        } else if (annotation.string == "interface") {
            pm_node_t *self = PMK::Self(annotation.typeLoc);
            helperNode = PMK::Send0(annotation.typeLoc, self, "interface!");
        } else if (annotation.string == "final") {
            pm_node_t *self = PMK::Self(annotation.typeLoc);
            if (self) {
                helperNode = PMK::Send0(annotation.typeLoc, self, "final!");
            }
        } else if (annotation.string == "sealed") {
            pm_node_t *self = PMK::Self(annotation.typeLoc);
            if (self) {
                helperNode = PMK::Send0(annotation.typeLoc, self, "sealed!");
            }
        }
 ...
 */
[[maybe_unused]] pm_node_t *maybeWrapBody(pm_node_t *body, core::LocOffsets loc) {
    if (body == nullptr) {
        // Create empty statements node
        pm_statements_node_t *stmts = PMK::allocateNode<pm_statements_node_t>();
        if (!stmts) {
            return nullptr;
        }
        *stmts = (pm_statements_node_t){.base = PMK::initializeBaseNode(PM_STATEMENTS_NODE),
                                        .body = {.size = 0, .capacity = 0, .nodes = nullptr}};
        stmts->base.location = PMK::convertLocOffsets(loc);
    ...
    *stmts = (pm_statements_node_t){.base = PMK::initializeBaseNode(PM_STATEMENTS_NODE),
    ...
    stmts->base.location = body->location;
...
[[maybe_unused]] bool containsExtendTHelper(pm_statements_node_t *body, const parser::Prism::Parser *prismParser,
    ...
    for (size_t i = 0; i < body->body.size; i++) {
        ...
        if (PMK::isT(constantPath->parent, *prismParser)) {
            return true;
        }
    }
...
[[maybe_unused]] void maybeInsertExtendTHelpers(pm_node_t **body, core::LocOffsets loc,
    ...
    pm_node_t *tHelpers = PMK::THelpers(loc);
    if (!tHelpers) {
        return;
    }

    pm_node_t *selfNode = PMK::Self(loc);
    if (!selfNode) {
        return;
    }

    pm_node_t *extendCall = PMK::Send1(loc, selfNode, "extend", tHelpers);
    if (!extendCall) {
        return;
    }
...
pm_node_t *SigsRewriterPrism::rewriteClass(pm_node_t *node) {
    ...
    switch (PM_NODE_TYPE(node)) {
        case PM_CLASS_NODE: {
            ...
            klass->body = maybeWrapBody(klass->body, loc);
            if (!helpers.empty()) {
                maybeInsertExtendTHelpers(&klass->body, loc, &parser, ctx);
        ...
        case PM_MODULE_NODE: {
            ...
            module->body = maybeWrapBody(module->body, loc);
            if (!helpers.empty()) {
                maybeInsertExtendTHelpers(&module->body, loc, &parser, ctx);
        ...
        case PM_SINGLETON_CLASS_NODE: {
            ...
            sclass->body = maybeWrapBody(sclass->body, loc);
            if (!helpers.empty()) {
                maybeInsertExtendTHelpers(&sclass->body, loc, &parser, ctx);
