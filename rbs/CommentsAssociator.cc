...
uint32_t CommentsAssociator::locateTargetLine(parser::Node *node) {
    ...
    typecase(
        node,
        [&](parser::String *lit) { result = core::Loc::pos2Detail(ctx.file.data(ctx), lit->loc.beginPos()).line; },
        [&](parser::DString *lit) { result = core::Loc::pos2Detail(ctx.file.data(ctx), lit->loc.beginPos()).line; },
        [&](parser::Array *arr) {
...
void CommentsAssociator::associateSignatureCommentsToNode(parser::Node *node,
                                                          absl::Span<const std::string_view> prefixes) {
    if (!rbsSignaturesEnabled) {
        return;
    }
    ...
    // Only update commentsByNode if we actually found comments
    if (!comments.empty()) {
        commentsByNode[node] = move(comments);
    }
}
...
void CommentsAssociator::walkNodes(parser::Node *node) {
    ...
    typecase(
        ...
        [&](parser::Class *cls) {
            associateSignatureCommentsToNode(node, {ANNOTATION_PREFIX});
            auto beginLine = core::Loc::pos2Detail(ctx.file.data(ctx), node->loc.beginPos()).line;
            consumeCommentsUntilLine(beginLine);
        ...
        [&](parser::DefMethod *def) {
            associateSignatureCommentsToNode(node, {RBS_PREFIX, ANNOTATION_PREFIX, MULTILINE_RBS_PREFIX});
        ...
        [&](parser::Send *send) {
            if (isVisibilitySend(send)) {
                ...
                associateSignatureCommentsToNode(send, {RBS_PREFIX, ANNOTATION_PREFIX, MULTILINE_RBS_PREFIX});
                auto beginLine = core::Loc::pos2Detail(ctx.file.data(ctx), node->loc.beginPos()).line;
                auto endLine = core::Loc::pos2Detail(ctx.file.data(ctx), node->loc.endPos()).line;
                consumeCommentsBetweenLines(beginLine, endLine, "send");
            } else {
...
CommentsAssociator::CommentsAssociator(core::MutableContext ctx, std::vector<core::LocOffsets> commentLocations)
    : ctx(ctx), commentLocations(commentLocations), commentByLine(),
      rbsAssertionsEnabled(ctx.state.cacheSensitiveOptions.rbsAssertionsEnabled),
      rbsSignaturesEnabled(ctx.state.cacheSensitiveOptions.rbsSignaturesEnabled) {
