 ...
 *
 * RBS type comments are formatted as `#: () -> void` for methods or `#:
 * Integer` for attributes.
 */
struct Comment {
  core::LocOffsets commentLoc; // The location of the comment in the file
  core::LocOffsets
      typeLoc;             // The location of the actual RBS content in the file
  std::string_view string; // The type string (excluding the `#: ` prefix)
                           // this is only a view on the string owned by the
                           // File.source() data.
};

/**
 * A collection of RBS type comments that collectively describe a method
 * signature, attribute type, or type assertion.
 */
...
public:
  ...
  RBSDeclaration(std::vector<Comment> comments)
   ...
   *
   * For multiline comments, this starts at the first comment's location and
   * ends at the last comment's location.
   *
   * ```
   * v starts from here
   * #: (Integer) ->
  ...
  /**
   * Returns the type location of the RBS declaration that covers the given
   * range. For example, if given the range of the `void` token:
   *
   * ```
   * #: (Integer) ->
   *    v starts from here
