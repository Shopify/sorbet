...
class ResolveTypeMembersAndFieldsWalk {
    ...
    static void resolveField(core::MutableContext ctx, ResolveFieldItem &job) {
        ...
        if (uid->kind == ast::UnresolvedIdent::Kind::Class) {
            ...
            if (minitest.exists()) {
                core::ClassOrModuleRef sym = ctx.state.lookupClassSymbol(
                    minitest, core::Names::Constants::Test());
            ...
            } else if (isMinitestTestClass && job.atTopLevel && ctx.owner.name(ctx) == core::Names::setup()) {
