...
void ClassOrModule::recordRequiredAncestorInternal(GlobalState &gs, ClassOrModule::RequiredAncestor &ancestor,
    ...
    for (auto &arg : ancestors.data(gs)->arguments) {
        ...
        ENFORCE(isa_type<ClassType>(type), "TupleType for requiredAncestors doesn't contain ClassType");
    ...
    auto &arg = gs.enterMethodArgumentSymbol(ancestor.loc, ancestors, core::Names::arg());
