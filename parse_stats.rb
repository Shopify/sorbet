...
if __FILE__ == $0
  ...
  total_fallbacks = all_entries.sum { |c| c[:after_count] }
  total_direct_desugared = total_nodes_in_repo - total_fallbacks

  formatted_repo_count = format_number(total_nodes_in_repo)
  formatted_fallbacks_count = format_number(total_fallbacks)
  formatted_direct_count = format_number(total_direct_desugared)

  puts "Direct desugaring summary:"
  puts "    Total nodes in repo:    #{formatted_repo_count}"
  puts "    Total direct desugared: %#{formatted_repo_count.length}s" % formatted_direct_count
  puts "    Total fallbacks:        %#{formatted_repo_count.length}s" % formatted_fallbacks_count
