...
# Compile sorbet first
echo "Compiling sorbet..."
...
# Find only .rb files in test/testdata/desugar and process each one
find test/testdata/desugar -name "*.rb" -type f | while read -r file; do
    ...
    if SORBET_SILENCE_DEV_MESSAGE=1 ./bazel-bin/main/sorbet --quiet --no-error-count --print=desugar-tree-raw-with-locs --parser=prism "$file" > "$output_file" 2>&1; then
        echo "  ✓ Success"
    else
        echo "  ✗ Failed"
        echo "    Output:"
