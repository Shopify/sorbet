...
void setGlobalStateOptions(core::GlobalState &gs, const options::Options &opts) {
    ...
    if (opts.cacheSensitiveOptions.rbsSignaturesEnabled || opts.cacheSensitiveOptions.rbsAssertionsEnabled) {
        if (auto e = gs.beginError(sorbet::core::Loc::none(), core::errors::Internal::DeprecatedOptionError)) {
            e.setHeader(
                "Options `--enable-experimental-rbs-signatures` and `--enable-experimental-rbs-assertions` have been "
                "combined into the `--enable-experimental-rbs` option. Please update your Sorbet config to use "
                "--enable-experimental-rbs instead.");
        }
