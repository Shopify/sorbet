...
class ParseError {
public:
  ParseError(pm_diagnostic_id_t id, const std::string &message,
             pm_location_t location, pm_error_level_t level)
...
class Parser final {
  // The version of Ruby syntax that we're parsing with Prism. This determines
  // what syntax is supported or not.
...
public:
  Parser(std::string_view sourceCode) : parser{}, options{} {
    pm_options_version_set(&options, ParsedRubyVersion.data(),
                           ParsedRubyVersion.size());

    pm_parser_init(&parser,
                   reinterpret_cast<const uint8_t *>(sourceCode.data()),
                   sourceCode.size(), &options);
  }
  ...
  static std::unique_ptr<parser::Node> run(core::MutableContext &ctx,
                                           core::FileRef file,
                                           bool directlyDesugar = true);
...
class ParseResult final {
  ...
  ParseResult(Parser &parser, pm_node_t *node,
              std::vector<ParseError> parseErrors)
      : parser{parser}, node{node, NodeDeleter{parser}}, parseErrors{
                                                             parseErrors} {}
  ...
  pm_node_t *getRawNodePointer() const { return node.get(); }
};
