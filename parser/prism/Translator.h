...
private:
    ...
    ast::ExpressionPtr desugarBreakNextReturn(core::LocOffsets location, pm_arguments_node *argsNode);
    ...
    // Desugar AndAsgn/OrAsgn operator assignments (e.g., `x &&= y`, `x ||= y`).
    template <typename SorbetAssignmentNode>
    ast::ExpressionPtr translateAndOrAssignment(core::LocOffsets location, ast::ExpressionPtr lhsExpr,
                                                ast::ExpressionPtr rhsExpr);

    template <typename PrismConstantNode, typename SorbetAssignmentNode>
    ast::ExpressionPtr translateConstantAssignment(pm_node_t *node, core::LocOffsets location);

    // Translate a constant path assignment, e.g. `A::B = 1`
    template <typename PrismConstantPathNode, typename SorbetAssignmentNode>
    ast::ExpressionPtr translateConstantPathAssignment(pm_node_t *node, core::LocOffsets location);
    ...
    ast::ExpressionPtr translateVariableAssignment(pm_node_t *node, core::LocOffsets location);

    // Translates an assignment to a method call, e.g. `x.y = z`
    template <typename PrismAssignmentNode, typename SorbetAssignmentNode>
    ast::ExpressionPtr translateSendAssignment(pm_node_t *node, core::LocOffsets location);

    // Translate operator assignment targeting a safe navigation call (e.g., `a&.b += 1`).
    template <typename PrismAssignmentNode, typename SorbetAssignmentNode>
    ast::ExpressionPtr translateCSendAssignment(PrismAssignmentNode *callNode, core::LocOffsets location,
    ...
    ast::ExpressionPtr translateConst(PrismLhsNode *node);
    ...
    // Desugar OpAsgn operator assignments
