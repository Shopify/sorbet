...
unique_ptr<parser::Node> Translator::translate(pm_node_t *node) {
    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_MODULE_NODE: { // Modules declarations, like `module A::B::C; ...; end`
            ...
            auto expr =
                MK::Module(location, declLoc, std::move(nameExpr), std::move(ancestors), std::move(bodyStore.value()));

            return make_node_with_expr<parser::Module>(std::move(expr), location, declLoc, move(name), move(bodyNode));
        }
