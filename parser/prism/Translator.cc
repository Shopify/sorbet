    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_BEGIN_NODE: { // A `begin ... end` block
            ...
            if (hasExpr(statements)) {
                ast::InsSeq::STATS_store args;
                args.reserve(statements.size());
                for (auto &statement : statements) {
                    args.emplace_back(statement->takeDesugaredExpr());
                }
        ...
        case PM_RETRY_NODE: { // The `retry` keyword
            return make_node_with_expr<parser::Retry>(MK::Retry(location), location);
        }
