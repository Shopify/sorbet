        ...
        case PM_AND_NODE: { // operator `&&` and `and`
            ...
            if (lhs->hasDesugaredExpr() && rhs->hasDesugaredExpr()) {
                ...
                if (preserveConcreteSyntax) {
                    ...
                    auto magicSend =
                        MK::Send2(location, MK::Magic(location.copyWithZeroLength()), core::Names::andAnd(), andAndLoc,
                                  std::move(lhsExpr), std::move(rhsExpr));
                    return make_node_with_expr<parser::And>(std::move(magicSend), location, std::move(lhs),
                                                            std::move(rhs));
                }
...
unique_ptr<parser::Node> Translator::translateIfNode(core::LocOffsets location, unique_ptr<parser::Node> predicate,
    ...
    if (hasExpr(predicate, ifTrue, ifFalse)) {
        ...
        return make_node_with_expr<parser::If>(move(if_node), location, move(predicate), move(ifTrue), move(ifFalse));
    }
