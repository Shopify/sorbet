...
unique_ptr<parser::Node> Translator::translate(pm_node_t *node) {
    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_IMPLICIT_REST_NODE: { // An implicit splat, like the `,` in `a, = 1, 2, 3`
            ...
            auto expr = MK::RestArg(location, MK::Local(restLoc, sorbetName));

            return make_node_with_expr<parser::Restarg>(std::move(expr), restLoc, sorbetName, restLoc);
        }
        ...
        case PM_REQUIRED_PARAMETER_NODE: { // A required positional parameter, like `def foo(a)`
            ...
            return make_node_with_expr<parser::Arg>(std::move(expr), location, name);
        }
        ...
        case PM_REST_PARAMETER_NODE: { // A rest parameter, like `def foo(*rest)`
            ...
            return make_node_with_expr<parser::Restarg>(std::move(expr), location, sorbetName, nameLoc);
        }
