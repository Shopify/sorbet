...
unique_ptr<NodeWithExpr<SorbetNode>> make_node_with_expr(ast::ExpressionPtr desugaredExpr, TArgs &&...args) {
    auto node = make_unique<NodeWithExpr<SorbetNode>>(std::forward<TArgs>(args)...);
    node->cacheDesugaredExpr(move(desugaredExpr));
    return node;
}
