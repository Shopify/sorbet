...
# Temporary files to hold desugar tree output
sorbet_output_prism=$(mktemp)
sorbet_output_sorbet=$(mktemp)

set +e  # Temporarily disable exit on error
# Run Sorbet with default parser
"$SORBET_BIN" --print=desugar-tree-raw-with-locs --parser=sorbet "$TEST_DIR/$TEST_FILE" > "$sorbet_output_sorbet" 2>/dev/null
# Run Sorbet with Prism parser
"$SORBET_BIN" --print=desugar-tree-raw-with-locs --parser=prism "$TEST_DIR/$TEST_FILE" > "$sorbet_output_prism" 2>/dev/null
