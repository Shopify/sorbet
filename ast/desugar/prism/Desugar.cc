...
ast::ExpressionPtr Desugarer::desugarMlhs(core::LocOffsets loc, PrismNode *lhs, ast::ExpressionPtr rhs) {
    ...
    auto processTarget = [this, &stats, &i, zloc, tempExpanded](pm_node_t *c) {
        ...
        if (PM_NODE_TYPE_P(c, PM_MULTI_TARGET_NODE)) {
            ...
            // Target of an indirect write to the result of a method call like `self.target1, self.target2 = 1, 2`
...
ast::Rescue::RESCUE_CASE_store Desugarer::desugarRescueCases(pm_rescue_node *firstRescueNode) {
    ...
    for (pm_rescue_node *rescueNode = firstRescueNode; rescueNode != nullptr; rescueNode = rescueNode->subsequent) {
        ...
        if (rescueNode->reference != nullptr) {
            if (PM_NODE_TYPE_P(rescueNode->reference, PM_LOCAL_VARIABLE_TARGET_NODE)) {
                ...
                if (PM_NODE_TYPE_P(rescueNode->reference, PM_CALL_TARGET_NODE)) {
                    auto *callTargetNode = down_cast<pm_call_target_node>(rescueNode->reference);
