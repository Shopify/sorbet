...
ast::ExpressionPtr Desugarer::desugarCallTargetNode(pm_call_target_node *node, core::LocOffsets cloc,
    ...
    if (PM_NODE_FLAG_P(node, PM_CALL_NODE_FLAGS_SAFE_NAVIGATION)) {
        auto body = [&rhs](ast::ExpressionPtr receiverTempLocal, core::LocOffsets parentLoc, core::NameRef methodName,
            ...
            arguments.emplace_back(move(rhs));
    ...
    return MK::Send(cloc, move(receiver), methodName, methodNameLoc, 1, move(arguments), flags);
}
    ...
    switch (PM_NODE_TYPE(node)) {
        ...
        case PM_FOR_NODE: { // `for x in a; ...; end`
            ...
            if (canProvideNiceDesugar) {
                ...
                } else if (PM_NODE_TYPE_P(forLoopVar, PM_CALL_TARGET_NODE)) { // single call target
                    // Desugar:
                    //     for self.foo in []
                    // into:
                    //     [].each do |forTemp$1|
